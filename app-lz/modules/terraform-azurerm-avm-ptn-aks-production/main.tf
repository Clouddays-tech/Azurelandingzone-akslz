...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  default_node_pool {
    ...
    max_pods               = 40
    min_count              = 2
    zones                  = [1, 3] # Specify the allowed zones
  ...
  depends_on = [azurerm_network_security_group.nsg, module.avm_res_network_virtualnetwork] ##Added depend_on feature to delete in order.
}
...
resource "azurerm_network_security_group" "nsg" {
  ...
  # Dynamic block for HTTP and HTTPS
  dynamic "security_rule" {
    for_each = [
      { name = "allow_https", port = 443, priority = 101 },
      { name = "allow_http", port = 80, priority = 102 }
    ]
    content {
